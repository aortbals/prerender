    ...
    var opts = {
        ...
        onExit: function(code, signal) {
            ...
            if (_this.options.onExit) {
                _this.options.onExit(code, signal);
            }
...
server.onRequest = function(req, res) {
    ...
    if (this.acceptingNewRequests) {
        util.log('getting', req.prerender.url);
        this.enqueuedRequestCount++;
        ...
        setTimeout(function() {
            util.log('waiting for server to accept requests...', req.prerender.url);
...
/**
 * Wait for all enqueued requests to finish, then restart phantom and notify
 * the server that we can process new requests.
 */
server._killPhantomJS = function() {
    if (this.restartingPhantomjs) { return }

    var _this = this;

    this.acceptingNewRequests = false;
    this.restartingPhantomjs = true;
    if (this.enqueuedRequestCount === 0) {
        util.log('restarting phantomjs...');
       require('tree-kill')(this.options.worker.process.pid, 'SIGTERM', function() {
            util.log('phantomjs restart complete.');
            _this.restartingPhantomjs = false;
            _this.acceptingNewRequests = true;
       });
   } else {
        util.log('waiting on', this.enqueuedRequestCount, 'requests to finish before restarting phantomjs...');
        setTimeout(function() {
            _this._killPhantomJS()
        }, 100)
   }
};
